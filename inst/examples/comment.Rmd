...
opts_chunk$set(warning=FALSE, message=FALSE, comment=NA, tidy=FALSE, verbose=TRUE, cache=FALSE)
````
...
``` {r comment-example-trajectories}
...
``` {r comment_figure1, dev="CairoPDF", fig.width=6, fig.height=5, echo=FALSE}
ggplot(dat) + geom_histogram(aes(value, y=..density..), binwidth=0.3, alpha=.5) +
 facet_wrap(~variable) + xlim(c(-1, 1)) + 
 geom_density(data=nulldat, aes(value), adjust=2) + xlab("Kendall's tau") + theme_bw()
...
``` {r comment_fig1, dev="CairoPS", fig.ext="eps", fig.width=6, fig.height=5, echo=FALSE}
ggplot(dat) + geom_histogram(aes(value, y=..density..), binwidth=0.3, alpha=.5) +
 facet_wrap(~variable) + xlim(c(-1, 1)) + 
 geom_density(data=nulldat, aes(value), adjust=2) + xlab("Kendall's tau") + theme_bw()
