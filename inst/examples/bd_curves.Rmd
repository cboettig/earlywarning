...
```{r include=FALSE, fig.keep="last"}
a <- 190
curve(d, lower, upper, lwd=1, add=T, col="red", lty=2)
a <- 200
curve(d, lower, upper, lwd=1, add=T, col="red", lty=2)
```
...
par(new=TRUE)
plot(dat$value, dat$time, type="l", xlim=c(lower,upper), xaxt="n", yaxt="n", ylim=c(.997*min(dat$time), max(dat$time)*1.005), xlab="", ylab="")
axis(4)
mtext("time", 4)
```
